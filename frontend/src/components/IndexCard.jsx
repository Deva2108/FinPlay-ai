...
export default function IndexCard({ symbol, value, change, percent, onClick }) {
  ...
  return (
    <div 
      onClick={onClick}
      className="bg-slate-900/60 p-4 rounded-xl border border-white/5 hover:bg-slate-800/80 transition-all cursor-pointer group relative overflow-hidden"
    >
      <div className="absolute inset-0 bg-gradient-to-br from-white/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
      
      <div className="relative z-10">
        <div className="text-[8px] font-black text-slate-500 uppercase tracking-widest mb-1 flex items-center justify-between">
          {symbol}
        ...
        <div className="flex justify-between items-end">
          ...
          <div className="flex flex-col items-end">
            ...
            <span className={`text-[7px] font-bold opacity-50 ${isPositive ? 'text-emerald-500' : 'text-rose-500'}`}>
              {change}
            </span>
          </div>
        </div>
